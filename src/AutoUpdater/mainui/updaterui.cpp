...
void UpdaterUI::initUI()
{
    ...
    m_teChangeLogViewer = new QTextEdit(this);
    m_teChangeLogViewer->setFocusPolicy(Qt::NoFocus);
    m_teChangeLogViewer->setWindowFlags(Qt::FramelessWindowHint);
    m_teChangeLogViewer->setGeometry(20, m_labelTitle->height() + 40, this->width() - 40,
                            this->height() - m_labelTitle->height() - 100);
    m_teChangeLogViewer->setStyleSheet("background-color:rgb(100, 100, 100);"
                                           "color:rgb(200, 200, 200)");
    m_teChangeLogViewer->setTextColor(QColor(200, 200, 200, 255));
    m_teChangeLogViewer->verticalScrollBar()->setStyleSheet("QScrollBar:vertical"
                                                                "{"
                                                                "width:8px;"
    ...
    m_labelLogTitle->setGeometry(m_teChangeLogViewer->x(),
                                 m_teChangeLogViewer->y() - 25,
                                 m_teChangeLogViewer->width() - 100,
                                 20);
    ...
    m_teChangeLogViewer->setVisible(true);
...
void UpdaterUI::updateUI()
{
    ...
    m_btnUpdate->setGeometry(20, m_teChangeLogViewer->height() + m_labelTitle->height() + 50, 70, 25);
    ...
    m_labelHaveNewVersionNotise->setGeometry(m_btnUpdate->x() + m_btnUpdate->width() + 10,
                                               m_btnUpdate->y(),
                                               m_teChangeLogViewer->width() - m_btnUpdate->width() - 10,
                                               m_btnUpdate->height());
...
void UpdaterUI::updatingUI()
{
    ...
    m_pbUpdating->setGeometry(m_teChangeLogViewer->x(), this->height() - 50,
                              m_teChangeLogViewer->width(), 20);
...
{
    m_teChangeLogViewer->append(QObject::tr("Checking for update "));
}
...
{
    m_teChangeLogViewer->clear();
    m_teChangeLogViewer->append(QObject::tr("Current version is ") + m_updater->getOldVersion());
    m_teChangeLogViewer->append("");
    m_teChangeLogViewer->append(QObject::tr("Version message: "));

    //Get the version information.
    QString _strVersionInfo = m_updater->getVersionInfo();
    if (_strVersionInfo.isEmpty())
    {
        m_teChangeLogViewer->setText(QObject::tr("Version information is missing!"));
        m_teChangeLogViewer->append(QObject::tr("Pleasse check network status, retry maybe solve."));
        m_teChangeLogViewer->append(QObject::tr("Or contact us: www.anycubic.com"));
    }
    else
    {
        m_teChangeLogViewer->append(_strVersionInfo.toLocal8Bit());
    }

    m_teChangeLogViewer->moveCursor(QTextCursor::Start);
}
...
void UpdaterUI::finishUpdate()
{
    ...
    m_teChangeLogViewer->append(QObject::tr("Update result: success."));
    m_teChangeLogViewer->append(QObject::tr("The update is complete, please restart!"));
...
void UpdaterUI::checkForUpdateError()
{
    ...
    m_teChangeLogViewer->clear();
    m_teChangeLogViewer->append(QObject::tr("Check for update result: failure!"));
    m_teChangeLogViewer->append("");
    m_teChangeLogViewer->append(QObject::tr("Error message: "));

    printErrorStack();

    m_teChangeLogViewer->append(QObject::tr("Please check the network status, retry maybe solve."));
    m_teChangeLogViewer->append(QObject::tr("Or contact us: www.anycubic.com"));
...
void UpdaterUI::updatingError()
{
    ...
    m_labelTitle->setText(QObject::tr("Failure"));
    m_teChangeLogViewer->append(QObject::tr("Update result: failure!"));
    m_teChangeLogViewer->append("");
    m_teChangeLogViewer->append(QObject::tr("Error message: "));
...
void UpdaterUI::printErrorStack()
{
    ...
    for(int i = 0 ; i < _vErrorStack.size(); i++)
    {
        ...
        m_teChangeLogViewer->append(_strErrorMsg);
    }
}
...
void UpdaterUI::on_updater_startDownloadPerFile(QString fileName)
{
    ...
    m_teChangeLogViewer->append(_strStartDownload);
}
...
void UpdaterUI::on_timer_checkForUpdate()
{
    ...
    if(_s_iDoct == 3)
        _s_iDoct = 0;
    m_teChangeLogViewer->setText(QObject::tr("Checking for update ") + QString::asprintf("%1").arg(_s_strTmp[_s_iDoct++]));
}
...
void UpdaterUI::language(bool bCh)
{
    if(bCh)
    {
        ...
        m_translator.load(":/translater/zh_en.qm");
